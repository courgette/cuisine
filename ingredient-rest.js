var express = require('express'),
    m = require('./middleware-ingredient');
 
// Instanciated module
module.exports = function () {
  var app = express();
   
  app.db = require('./db-ingredient')();
...
/*app.configure('test', function () {
  ...
  app.get( '/', m.dbAction(app.db, 'fetchAll', function (ids) {
    return ids.map(function (id) {
      return app.route + (app.route.charAt(app.route.length-1) == '/' ? '' : '/') + 'ingredient/' + id;
    });
  }));
  app.get( '/ingredient/:id', m.dbAction(app.db, 'fetchOne'));
  app.put( '/ingredient/:id', m.dbAction(app.db, 'save'));
  app.del( '/', m.dbAction(app.db, 'deleteAll'));
  app.del( '/ingredient/:id', m.dbAction(app.db, 'deleteOne'));
