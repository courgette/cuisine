var express = require('express'),
    m = require('./middleware-shop');
 
// Instanciated module
module.exports = function () {
  var app = express();
   
  app.db = require('./db-shop')();
...
/*app.configure('test', function () {
  ...
  app.get( '/', m.dbAction(app.db, 'fetchAll', function (ids) {
    return ids.map(function (id) {
      return app.route + (app.route.charAt(app.route.length-1) == '/' ? '' : '/') + 'shop/' + id;
    });
  }));
  app.get( '/shop/:id', m.dbAction(app.db, 'fetchOne'));
  app.put( '/shop/:id', m.dbAction(app.db, 'save'));
  app.del( '/', m.dbAction(app.db, 'deleteAll'));
  app.del( '/shop/:id', m.dbAction(app.db, 'deleteOne'));
